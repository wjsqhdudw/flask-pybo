...
def modify(answer_id):
    ...
    if request.method == 'POST':    # 전송 방식 POST
        form = AnswerForm()
        if form.validate_on_submit():
            form.populate_obj(answer)
            answer.modify_date = datetime.now() # 수정 일자 저장
            db.session.commit()
            return redirect('{}_answer_{}'.format(url_for('question.detail', question_id=answer.question.id), answer.id))
    else :
